...
SELECT s.street,
AVG(s.safetyrating) AS AvgSafe_rate
FROM BIT_DB.CityBikeLanes s
GROUP BY s.street
)
SELECT street,
AvgSafe_rate,
...
CASE
WHEN AVG(safetyrating) OVER (PARTITION BY street) >=4.0 THEN "Leave As-Is"      #DO NOT USE ALIAS
WHEN AVG(safetyrating) OVER (PARTITION BY street) <2.5 THEN "Remove"            #DO NOT USE ALIAS
ELSE "Improvements Needed"
END AS "Recommendation"
FROM BIT_DB.CityBikeLanes
ORDER BY Average_safety_rating DESC                                             #ORDERED
